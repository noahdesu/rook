...
func scheduleMonitor(mon *monConfig, nodeZones [][]NodeUsage) *NodeUsage {
	...
	for zi := range nodeZones {
		...
		var zoneNodeChoice *NodeUsage = nil
		...
		for ni := range nodeZones[zi] {
			...
			if !nodeUsage.MonValid {
				logger.Infof("XXX skipping invalid node %s for mon scheduling", nodeUsage.Node.Name)
				continue
			}
			...
			if zoneNodeChoice == nil || nodeUsage.MonCount < zoneNodeChoice.MonCount {
				logger.Infof("XXX schedmon: considering node %s with mon count %d",
					nodeUsage.Node.Name, nodeUsage.MonCount)
				zoneNodeChoice = nodeUsage
			}
		}

		// We identified _a_ valid node the zone. should we choose it?
		if zoneNodeChoice != nil {
			if zoneMonCount == 0 {
				...
				logger.Infof("XXX schedmon: considering node %s from empty zone",
	...
	if nodeChoice != nil {
		logger.Infof("XXX schedmon: scheduling mon %s on node %s",
			mon.DaemonName, nodeChoice.Node.Name)
	} else {
		logger.Infof("XXX schedmon: no suitable node found for mon %s",
			mon.DaemonName)
	}
...
func (c *Cluster) assignMons(mons []*monConfig) error {
	...
	// necessarily optimal: it does not try to move existing monitors.
	for _, mon := range mons {

		// monitor is already assigned to a node. nothing to do
		if _, ok := c.mapping.Node[mon.DaemonName]; ok {
			logger.Infof("XXX mon %s already assigned to a node, no need to assign", mon.DaemonName)
			continue
		}
		...
		if nodeChoice == nil {
			return fmt.Errorf("XXX no valid nodes available for mon placement")
		}
		...
		nodeChoice.MonCount += 1

		logger.Infof("XXX mon %s assigned to node %s", mon.DaemonName, nodeChoice.Node.Name)
