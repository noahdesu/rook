...
func (c *Cluster) startMon(m *monConfig, hostname string) error {
	...
	sts := c.makeStatefulSet(m, hostname)
	logger.Debugf("Starting mon: %+v", sts.Name)
	_, err = c.context.Clientset.AppsV1().StatefulSets(c.Namespace).Create(sts)
	if err != nil {
		if !errors.IsAlreadyExists(err) {
			return fmt.Errorf("failed to create mon statefulset %s. %+v", m.ResourceName, err)
		}
		logger.Infof("statefulset for mon %s already exists. updating if needed", m.ResourceName)
		...
		if err := UpdateCephStatefulSetAndWait(c.context, sts, c.Namespace, daemonType, m.DaemonName, cephVersionToUse); err != nil {
			return fmt.Errorf("failed to update mon statefulset %s. %+v", m.ResourceName, err)
		}
	} else {
		logger.Infof("created mon statefulset: %+v", sts.Name)
	}
