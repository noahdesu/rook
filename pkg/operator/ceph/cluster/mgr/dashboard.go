...
func (c *Cluster) configureDashboardModule(dashboardPort int) error {
	// url prefix
	hasChanged, err := client.MgrSetAllConfig(c.context, c.Namespace, c.cephVer, "mgr/dashboard/url_prefix", c.dashboard.UrlPrefix)
	if err != nil {
		return err
	}
	...
	changed, err := client.MgrSetAllConfig(c.context, c.Namespace, c.cephVer, "mgr/dashboard/server_port", port)
	if err != nil {
		return err
	}
	...
	changed, err = client.MgrSetAllConfig(c.context, c.Namespace, c.cephVer, "mgr/dashboard/ssl", ssl)
	if err != nil {
		return err
	}
	hasChanged = hasChanged || changed
