...
func TestVersionName(t *testing.T) {
	assert.Equal(t, "nautilus", fmt.Sprintf("%s", &Nautilus))
	assert.Equal(t, "mimic", fmt.Sprintf("%s", &Mimic))
	assert.Equal(t, "luminous", fmt.Sprintf("%s", &Luminous))
	assert.Equal(t, unknownVersionString, fmt.Sprintf("%s", &CephVersion{-1, 0, 0}))
}
