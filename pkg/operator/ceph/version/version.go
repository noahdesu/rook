...
func ExtractCephVersion(src string) (CephVersion, error) {
	...
	if m == nil {
		return CephVersion{}, fmt.Errorf("failed to parse version from: %s", src)
	}

	major, err := strconv.Atoi(m[1])
	if err != nil {
		return CephVersion{}, fmt.Errorf("failed to parse version major part: %s", m[0])
	}

	minor, err := strconv.Atoi(m[2])
	if err != nil {
		return CephVersion{}, fmt.Errorf("failed to parse version minor part: %s", m[1])
	}

	extra, err := strconv.Atoi(m[3])
	if err != nil {
		return CephVersion{}, fmt.Errorf("failed to parse version extra part: %s", m[2])
	}
