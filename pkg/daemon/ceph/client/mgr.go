...
func MgrSetAllConfig(context *clusterd.Context, clusterName string, cephVersion version.CephVersion, key, val string) (bool, error) {
	return MgrSetConfig(context, clusterName, "", cephVersion, key, val, false)
}

// MgrSetConfig applies a setting for a single mgr daemon
func MgrSetConfig(context *clusterd.Context, clusterName, mgrName string, cephVersion version.CephVersion, key, val string, force bool) (bool, error) {
	var getArgs, setArgs []string
	mgrID := fmt.Sprintf("mgr.%s", mgrName)
	if cephVersion.IsRelease(version.Luminous) {
		...
		if force && cephVersion.AtLeast(version.Nautilus) {
